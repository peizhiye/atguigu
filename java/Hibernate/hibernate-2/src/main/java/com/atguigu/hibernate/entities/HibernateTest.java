...
//		session.save(news);
	 ...
	 * 2. 了解: OID 值等于 id 的 unsaved-value 属性值的对象, 也被认为是一个临时对象
	 */
	@Test
	 ...
	 * 的 commit() 方法时, 会先执行 session 的 flush 方法. 若发现持久化对象状态有变化，才会发起 update 操作
	 * 2. 更新一个游离对象, 需要显式的调用 session 的 update 方法. 可以把一个游离对象
	 * 变为持久化对象
	 * 
	 * 需要注意的:
	 * 1. 无论要更新的游离对象和数据表的记录是否一致, 都会发送 UPDATE 语句. 
	 *    如何能让 update 方法不再盲目的触发 update 语句呢 ? 在 .hbm.xml 文件的 class 节点设置
	 *    select-before-update=true (默认为 false). 但通常不需要设置该属性, 要与触发器配合使用时才设置该属性. 
	 * 
...
//		System.out.println(news);
	 ...
	 * 
	 * 1. 执行 get 方法: 会立即加载对象. 
	 *    执行 load 方法, 若不使用该对象, 则不会立即执行查询操作, 而是返回一个代理对象
	 *    
...
//		session.close();
	 ...
	 * 
	 * 和 save() 的区别 : 
	 * 在调用 persist 方法之前, 若对象已经有 id 了, 则不会执行 INSERT, 而是抛出异常
	 */
	@Test
