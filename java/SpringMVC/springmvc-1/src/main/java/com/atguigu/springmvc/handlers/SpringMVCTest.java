...
public class SpringMVCTest {
	 ...
	 * 2. 解析请求处理器的目标参数（resolveModelAttribute 方法）, 实际上该目标参数来自于 WebDataBinder 对象的 target 属性
	 ...
	 * 
	 * ②. 确定 target 属性:
	 * 	> 在 implicitModel 中查找 objectName 对应的属性值. 若存在, ok
	 * 	> *若不存在: 则验证当前 Handler 是否使用了 @SessionAttributes 进行修饰, 若使用了, 则尝试从 Session 中
	 * 获取 objectName 所对应的属性值. 若 session 中没有对应的属性值, 则抛出了异常.
	 * 	> 若 Handler 没有使用 @SessionAttributes 进行修饰, 或 @SessionAttributes 中没有使用 value 值指定的 key
	 * 和 objectName 相匹配, 则通过反射创建了 POJO 对象
	 * 
	 * 2). SpringMVC 把表单的请求参数赋给了 WebDataBinder 的 target 对应的属性（doBind 方法）.
	 * 3). *SpringMVC 会把 WebDataBinder 的 objectName 和 target 给到 implicitModel.
	 * 近而传到 request 域对象中. 
